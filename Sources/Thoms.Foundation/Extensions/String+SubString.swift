...
extension String {
	...
	@inlinable
	public var rParseNumberWithDigits: ParsingResult<DefaultPaddedInt, String> {
        var array = splitByNumber
		guard !array.isEmpty else { return ParsingResult(self, Optional<DefaultPaddedInt>.none) }
		guard let match = DefaultPaddedInt(array.popLast()!) else { return ParsingResult(self, Optional<DefaultPaddedInt>.none) }
        return ParsingResult(array.joined(), match)
    }
	...
	@inlinable
	public var lParseNumberWithDigits: ParsingResult<DefaultPaddedInt, String> {
        var array = splitByNumber
        guard !array.isEmpty else { return ParsingResult(self, Optional<DefaultPaddedInt>.none) }
        guard let match = DefaultPaddedInt(array.removeFirst()) else { return ParsingResult(self, Optional<DefaultPaddedInt>.none) }
        return ParsingResult(array.joined(), match)
    }
}
