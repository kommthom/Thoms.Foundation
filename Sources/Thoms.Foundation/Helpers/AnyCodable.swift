...
//  Adapted by Thomas Benninghaus on 02.07.24.
//
...
//        case typeNameNotFound, archiveTypeNotFound, typeNameAlreadyExists
//    }
//    
//    private static var typeMappings = [String : CodableWithTypeName.Type]()
//    
//    private static let closuresLock = NSRecursiveLock()
//    private static var basicTypeRegistered = false
//    
...
//    public let value: Any
//    
//    public static func registerType<T: CodableWithTypeName>(_ type: T.Type) throws {
//        closuresLock.lock()
//        defer { closuresLock.unlock()}
//        guard let typeName = type.typeName else { throw Errors.typeNameNotFound }
//        guard typeMappings[typeName] == nil else { throw Errors.typeNameAlreadyExists }
...
//    
//    private static func RegisterBasicTypes() {
//        guard basicTypeRegistered == false else { return } // Already registered
//        closuresLock.lock()
//        defer { closuresLock.unlock() }
//        guard basicTypeRegistered == false else { return } // Double-check lock
//
//        try? registerType( DateRelative.self )
//        try? registerType( DateRelative.ComponentDate.self )
//        try? registerType( DateRelative.ComponentTime.self )
//        try? registerType( DateRelative.ComponentDate.ComponentYear.self )
//        try? registerType( DateRelative.ComponentDate.ComponentMonth.self )
//        try? registerType( DateRelative.ComponentDate.TypedWeek.self )
//        try? registerType( DateRelative.ComponentDate.TypedDay.self )
//        try? registerType( DateRelative.ComponentTime.Hour.self )
//        try? registerType( DateRelative.ComponentTime.QuarterHour.self )
//        try? registerType( TimeZone.self )
//
//        basicTypeRegistered = true
//    }
//
...
//            default:
//                let typeName = String(describing: type(of: value))
//                guard AnyCodable.typeMappings[typeName] != nil else { throw EncodingError.invalidValue(value, EncodingError.Context(codingPath: [], debugDescription: "Not registered type: \(typeName)")) }
//                self.typeName = typeName
//        }
//    }
//    
//    public init(value: CodableWithTypeName) throws {
//        AnyCodable.RegisterBasicTypes()
//        self.value = value
//        self.typeName = type(of: value).typeName ?? String(describing: type(of: value))
//    }
//    
//    public init(value: Array<Any>) throws {
//        AnyCodable.RegisterBasicTypes()
...
//        self.typeName = typeName
//        switch typeName {
//            case AnyCodable.ArrayTypeName:
//                var unkeyedContainer = try container.nestedUnkeyedContainer(forKey: .value)
//                self.value = try AnyCodable.decodeAnyArray(from: &unkeyedContainer)
//            case AnyCodable.SetTypeName:
//                var unkeyedContainer = try container.nestedUnkeyedContainer(forKey: .value)
//                self.value = try AnyCodable.decodeAnySet(from: &unkeyedContainer)
//            case AnyCodable.DictionaryTypeName:
//                var unkeyedContainer = try container.nestedUnkeyedContainer(forKey: .value)
//                self.value = try AnyCodable.decodeAnyDictionary(from: &unkeyedContainer)
//            default:
//                guard let mappedType = AnyCodable.typeMappings[typeName] else { throw Errors.archiveTypeNotFound }
//                self.value = try mappedType.init(from: decoder)
//        }
//    }
//    
//    public func encode(to encoder: Encoder) throws {
...
//                try AnyCodable.encodeAnyDictionary(value as! [AnyHashable: Any], to: &unkeyedContainer)
//            default:
//                guard let _ = AnyCodable.typeMappings[typeName] else { throw Errors.archiveTypeNotFound }
//                if let encodable = value as? Encodable {
//                    try encodable.encode(to: container.superEncoder(forKey: .value))
//                }
//        }
//    }
//    
...
//            } else if let codableValue = value as? Dictionary<AnyHashable, Any> {
//                try container.encode(AnyCodable(value: codableValue))
//            } else if let codableValue = value as? CodableWithTypeName {
//                try container.encode(AnyCodable(value: codableValue))
//            } else {
//                throw EncodingError.invalidValue(value, EncodingError.Context(codingPath: container.codingPath, debugDescription: "Value unsupported type \(type(of: value))"))
//            }
//        }
//    }
//    
...
//                set.insert(anyHashableValue)
//            } else {
//                throw DecodingError.dataCorruptedError(in: container, debugDescription: "Expected hashable value in Set. \(type(of: value)) is not Hashable")
//            }
//        }
//        return set
//    }
...
//    private static func encodeAnyDictionary(_ dict: [AnyHashable: Any], to container: inout UnkeyedEncodingContainer) throws {
//        for (key, value) in dict {
//            guard let codableKey = key.base as? Codable else { throw EncodingError.invalidValue(key.base, EncodingError.Context(codingPath: container.codingPath, debugDescription: "Key unsupported type \(type(of: key.base))")) }
//            try container.encode(AnyCodable(value: codableKey))
//            if let codableValue = value as? Set<AnyHashable> {
//                try container.encode(AnyCodable(value: codableValue))
//            } else if let codableValue = value as? Array<Any> {
...
//    private static func decodeAnyDictionary(from container: inout UnkeyedDecodingContainer) throws -> [AnyHashable: Any] {
//        // We're expecting to get pairs. If the container has a known count, it had better be even; no point in doing work if not.
//        if let count = container.count { guard count % 2 == 0 else { throw DecodingError.dataCorruptedError(in: container, debugDescription: "Expected collection of key-value pairs; encountered odd-length array instead.") } }
//        var dict = [AnyHashable: Any]()
//        while !container.isAtEnd {
//            guard let key = try container.decode(AnyCodable.self).value as? AnyHashable else { throw DecodingError.dataCorruptedError(in: container, debugDescription: "Expected Hashable key in Dictionary") }
//            guard !container.isAtEnd else { throw DecodingError.dataCorruptedError(in: container, debugDescription: "Unkeyed container reached end before value in key-value pair.") }
//            let value = try container.decode(AnyCodable.self).value
//            dict[key] = value
//        }
//        return dict
